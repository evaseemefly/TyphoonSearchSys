...
export default class TideChartView extends Vue {
	...
	initCharts(yList: number[], xList: Date[]): void {
		const that = this
			...
			const option = {
				...
				tooltip: {
					...
					formatter: (params: any): string => {
						let content = '---'
						const surge = params[0].data

						content = `当前潮值:${surge}</br>`
	...
	async onStationCode(val: {
		...
		if (val.stationName !== DEFAULT_STATION_NAME) {
			...
			this.loadStationSurgeDataList(
				...
				MenuType.all,
				() => {
					self.initCharts(self.forcastValList, self.forecastDtList)
				}
			)
		...
		loadStationDetailDataList({
			...
			*/
				...
				res.data.forEach((temp) => {
					self.forcastValList.push(temp.val_forecast)
					self.forecastDtList.push(new Date(temp.occurred))
				})
				const surgeMax = Math.max(...self.forcastValList)
				const surgeMin = Math.min(...self.forcastValList)
